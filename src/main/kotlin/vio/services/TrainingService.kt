...
class TrainingService(
    ...
    fun calculatePoints(term: String?, answer: String, hints: Int): Int {
        if (term != null && answer.isNotBlank()) {
            if (term == answer) {
                ...
                if (LevenshteinDistance.getDefaultInstance().apply(term, answer) > maxErrors) {
    ...
    fun updatePoints(user: User, phrase: Phrase, mode: String, points: Int) {
        val userPhrase = userPhraseRepository.findByUserAndPhrase(user, phrase) ?: return

        val updatedUserPhrase = if (points != 0) {
            val auditionPoints =
                if (mode == "listening") getFixedPoints(userPhrase.auditionPoints + points) else userPhrase.auditionPoints
            val memoryPoints =
                if (mode == "writing") getFixedPoints(userPhrase.memoryPoints + points) else userPhrase.memoryPoints
            userPhrase.copy(
